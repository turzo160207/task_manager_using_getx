...
class _ResetPasswordScreenState extends State<ResetPasswordScreen> {
  ...
  Future<void> _passwordReset() async {
    ...
    if(_passwordTEController.text.trim() != _confirmPasswordTEController.text.trim()){
      showCustomSnackBar("The passwords don't match", true);
    }
    else if (result) {
      showCustomSnackBar('Password reset successful', false);
      Get.offAll(
            () => SignInScreen(),
        predicate: (route) => false,
      );
    } else {
      showCustomSnackBar(resetPasswordController.errorMessage!, true);
    }
  }
